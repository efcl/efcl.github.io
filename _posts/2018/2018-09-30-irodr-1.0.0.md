...
RSSの管理自体は[Inoreader](https://www.inoreader.com/)で行って、RSSのクロールなどは[Inoreader](https://www.inoreader.com/)が行っていて、API経由でRSSを取得しているだけです。
...
LDRライクなショートカットでフィードを移動するようなフォーカスコントールとかも行えました。（まあまあ無理やり）
...
このAPIを使ってバックエンドでタブを開いたり、NGフィルターを作ったりするUserScriptもあります。
