...
この場合は、以下のようなDSL(mercuryではこれをもう少し簡単にする[Raynos/virtual-hyperscript](https://github.com/Raynos/virtual-hyperscript "Raynos/virtual-hyperscript")というモジュールを持っている)を使い、Virtual DOMを組み立てる感じですね。
...
しかし、考えてみるとmercuryではいわゆるデータバインディング的な事をするのに本物のDOMがでてこないため、殆どのエラーがJavaScriptに落ちてきたり、テストが書きやすかったりします。
(何が言いたいのかよく分からなくなってますが、要は見えない処理が少ないので、見通しがいいですねという話)
