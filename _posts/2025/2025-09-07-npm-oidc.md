...
- トークンの手動ローテーションが必要
- 必要以上に広い権限を持つことが多い([granular access tokens](https://docs.npmjs.com/about-access-tokens#about-granular-access-tokens)ではscopeを制限できますが、monorepoの場合はこれを設定/維持するのが難しいのでorgというでかい単位になりがちです)
...
2. `release.yml`: リリースPRがマージされたら publish して GitHub Release を作る
  - Trusted Publisherを設定には `release.yml` のみを指定する
...
この変更によって、GitHub Actionsからnpmへのパッケージ公開においては、長期間有効なnpmトークンを使うよりはOIDCを使用することで確実によくなってる。
GitHub Actionsのsecretsにnpm tokenがなくなった。もしOIDCのtokenが漏れても短時間で無効になるため、攻撃コストが上がっている。

一方で、ローカルからnpmへpublishするのに比べて、GitHub Actionsからnpmにpublishすると若干リスクは変わる部分がある。
具体的には、従来はGitHubアカウントの乗っ取りがあっても問題はなかった。npm tokenの漏洩さえなければnpmにpublishされることはないためです。
...
GitHub Actionsの[Environment Protection Rules](https://docs.github.com/en/actions/deployment/targeting-different-environments/using-environments-for-deployment#environment-protection-rules)を設定することで、`release.yml`が動くときにもう一段階の認証を挟んでいる。
...
- ⚠️ npm側のOIDC設定でEnvironmentを`npm`に設定しまう

```yaml
# .github/workflows/release.yal
jobs:
  publish:
    runs-on: ubuntu-latest
...
`release.yal`自体が変更されることを防ぐため、次の対策を実装している。
...
.github/workflows/*.yml @your-username
.github/workflows/*.yaml @your-username
```

2. **Branch Protection Rules**: メインブランチに対する直接プッシュを禁止し、必ずPull Requestを経由するルールを設定

これらの対策により、攻撃者がプロテクションルールを回避するには必ずPull Requestを通して何かを変更する必要がある。
Pull Requestは公開で行われるため、攻撃が可視化され、攻撃者にとって攻撃がしにくくなる。
...
現在のOIDC化とEnvironment + Protection Ruleset(CODEOWNERS)の組み合わせにより、次の効果が得られている。

- 攻撃者は攻撃に公開Pull Requestを経由する必要がある
- 水面下でのワークフロー改変やGitHub Actionsのトークン(`secrets.GITHUB_TOKEN`)の悪用が困難になった
...
そのため、影響が大きいパッケージは"Require two-factor authentication and disallow tokens"が有効になっている状態が良い。
（これ自体はOIDC化とは別でできるが、毎回認証はさむのは面倒なのでOIDCとセットだとやりやすくなる。）
