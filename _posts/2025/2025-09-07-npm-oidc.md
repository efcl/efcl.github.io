...
- ⚠️ npm側のOIDC設定でEnvironmentを`npm`に設定する必要がある
...
2. **Branch Protection Rules**: メインブランチに対する直接プッシュを禁止し、必ずPull Requestを経由するルールを設定 + "Require review from Code Owners"を追加
...
Pull Requestは公開で行われるため、攻撃が可視化され、攻撃者にとって攻撃がしにくくなる効果があります。
