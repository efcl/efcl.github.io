...
タスク管理ツールは今まで[Producteev](https://www.jivesoftware.com/producteev/)や[OmniFocus](https://www.omnigroup.com/omnifocus/)などを使ってきました。
...
しかし、この[User-owned project boards](https://docs.github.com/en/free-pro-team@latest/github/managing-your-work-on-github/about-project-boards)は25リポジトリの制限あったりリポジトリを明示的に紐付ける必要があるため、人軸のIssue管理には扱いにくそうでした。
...
[My-Board-For-Github(MYFG)](https://github.com/azu/my-board-for-github)は悪くはなかったのですが、問題があっても[ものすごい複雑なGraphQLクエリ](https://github.com/azu/my-board-for-github/blob/5d1c2f6edbda3c8c1dd61dd811c514557bfdde0b/src/queries/github-query.ts#L333-L389)を書いているので、最初の表示が遅いという問題がありました。
また、Board自体にIssueを作れないので、あんまり気軽にIssueを作りにくくてあくまで管理ツールって感じでした。
...
CR IssueはCross Reference Issueの略で、 `CR` ラベルがついていて、Issueのボディに別IssueのURLが書かれているものを示しています。
...
![Template](https://raw.githubusercontent.com/azu/missue/master/docstemplate.png)
...
IFTTT、zapier、APIを使ってGitHubにデータを流すことが比較的簡単です。
...
または[Sponsorware](https://github.com/sponsorware/docs)的にsponsorship thresholdを設定(登録数が70人にいったら公開)してみるのも面白いのかも。
...
- GitHubコメント → GASサーバにリクエスト → GASで `ScriptApp.newTrigger("commentToGitHub").timeBased().at(triggerDate).create()` を作成 → `commentToGitHub`でコメントする
...
GitHub IssueベースならAPIやActionsで拡張できるので、その辺は柔軟性があるのが便利です。
一方で、GitHub Issueは通知系や日付管理系(マイルストーンぐらいしか日付データがない)が弱めなので、その辺はGoogle App Script(GAS)やカレンダーアプリなどと連携して補う必要がある気がします。
