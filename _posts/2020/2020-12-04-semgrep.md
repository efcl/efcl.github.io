...
これらに`eval(引数)`にマッチする正規表現を書くのは結構難しいですが、Semgrepでは`eval(...)`というパターンで検索できます。
...
例えば、semgrepを使って`eval()`を使っているコードを検索したい場合は次のようにすれば検索できます。
...
`requests.get("=~/dev\./i")` のような 関数とその引数が`dev.`というように正規表現を組み合わせたマッチもできます。
見た目に近いパターンマッチができます。
...
Semgrepではyamlでのconfigファイルを使って、複数のパターンやもっと複雑な組み合わせも表現できます。
...
この変数を置換後の `fix:` に入れれば、元のマッチで一部をキャプチャして置換に使えます。
...
[HonKit](https://github.com/honkit/honkit)というドキュメントや書籍を作成ツールをつくっています。
...
またギリギリ文字列置換でもできそうな雰囲気もありますが、文字列マッチだと誤検知したりすると面倒そうでした。

そこで、[Semgrep](https://github.com/returntocorp/semgrep)を使って労力はあまりに使わずにこの置換をルールとして定義して、マイグレーションしていくことにしました。
...
また、この正規表現での置換結果はコードとしてパースできない状態に置換もできる点も結構自由度を挙げています。

最初のルール(`id: model`)である`const $X = Immutable.Record(...);` → `class $X extends Immutable.Record(...){`は、単独手も見ると `class {` に対する `}` が存在しないので構文的にはおかしいです。
...
すべてをASTでマッチと置換にすると覚える構文増えるので、コスパが良い感じです。
...
この`pattern-not-inside`によって、次のような場合は除外されます。
...
[CodeQL](https://securitylab.github.com/tools/codeql)との比較もありますが、CodeQLはデータフロー解析をするGitHubが買収したツールです。
