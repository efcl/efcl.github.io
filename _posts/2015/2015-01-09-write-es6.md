...
少し注意したほうがいいのは、上記のツールは構文(`Class`や`module`など)をサポートという意味が多くて(そうじゃないとシンタックスエラーになるので)、`Array.form`や`Promise`といったAPIは別のpolyfill(またはruntimeなどと言わわれるもの)を使いサポートしている違いがあります。

こういった変換ツールを使う際は構文のサポートを求める感じで使うのがいいと思います。
...
ES6の構文を使うことでコードもシンプルになりますが、それを公開するためには変換ツールを使わないといけないというのが現状だと思います(一種のAltJSみたいなもの)
