...
6to5 は Babel へリネームされたので、6to5をBabelと置き換えて読んで下さい。
...
- `src/`: ES6で書かれたコード
- `lib/`: 6to5で`src/`以下のコードを変換したES5のコード
...
ES6 -> ES5の変換には[6to5](https://6to5.org/ "6to5")を使っています。`devDependencies`で入れておいて、npm run-scriptで実行するようにすればglobalに入れる必要はありません。
...
```
mocha --compilers js:6to5/register test/**/*.js
```
