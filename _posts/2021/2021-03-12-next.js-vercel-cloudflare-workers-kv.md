...
- SpreadSheet API: [Usage Limits  |  Sheets API  |  Google Developers](https://developers.google.com/sheets/api/limits)
    - 秒間リクエストが多発しなければほぼ問題おきなさそう
- Cloudflare Workers KV: [Limits · Cloudflare Workers docs](https://developers.cloudflare.com/workers/platform/limits)
    - Readはゆるい(100K/day)なので、Writeが多くなければFreeでも十分
