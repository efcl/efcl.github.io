...
- `Store#getState`を単純にStateそのものを返すようにする単純化
- 型安全なStoreGroup

次のように`context.getState()`(`StoreGroup#getState()`)した結果が、最初にコンストラクタで渡した`Store`と`State`の関係でマッピングされて自動的に型付いたstateのオブジェクトを返すようになっています。
...
Storeに`receivePayload(payload: Payload)`というメソッドを実装するとAlminのライフサイクルにおいてそれが呼びされるようになりました。
(Reactの`componentWillReceiveProps(nextProps)`と同じですね)

これを導入した経緯は以下のIssueにも書いているのですが、[クライアントサイドでCQRSをちゃんとやろう](http://azu.github.io/slide/2016/react-meetup/large-scale-javascript.html "複雑なJavaScriptアプリケーションを考えながら作る話")としたのがAlminの開発目的の一つなので、その目的を再確認した結果が生まれました。
...
またViewはどのようなUIを目指すかによって色々な細かいStateがでてきますが、それをStore/Stateで吸収して、Domain modelが変にUIを意識して書くケースをかなり減らせていたと思います。

先ほどのスライドでいうとDomainはWrite層で、StoreはRead層という分離ができていました。
...
Almin 0.12 + TypeScriptならStoreは型安全に書けるようなったと思います。
...
これを次期Alminでは次のように型安全に書けるようにしたいと思っています。
